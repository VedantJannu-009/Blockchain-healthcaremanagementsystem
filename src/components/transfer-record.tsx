...
interface ShareRecordProps {
  ...
  transferRecord: (data: ShareRecordType) => void;
}

const TransferRecord = ({ contract, transferRecord }: ShareRecordProps) => {
  ...
  return (
    <Form {...form}>
      <form
        onSubmit={form.handleSubmit((data) => transferRecord(data))}
        className="space-y-6"
      >
        <FormField
